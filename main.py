...
def register_chat(message_thread_id, chat_id_sender):
    with open("chat_ids.json", "r", encoding="utf-8") as d:
        data = json.load(d)
...
def message_get(message):
    ...
    if message.chat.id == config_data["forum_id"] and message.message_thread_id:
        target_user_id = None
        ...
        if text_lower.startswith("/blacklist"):
            ...
            return
        if text_lower.startswith("/unblacklist"):
    ...
    if user_id in chat_ids:
        thread_id = chat_ids[user_id]
        if user_id in config_data["blacklist"]:
            bot.send_message(config_data["forum_id"], message_thread_id=thread_id, text=f"User {user_id} blocked")
            return
        bot.send_message(config_data["forum_id"], message_thread_id=thread_id, text=message.text)
        return
